...
public class PipeOverallCheckPager extends MenuDetailBasePager implements ViewPager.OnPageChangeListener{
	...
	String[] indicatorItem = {"宏观检查记录","安全保护装置检查情况","管道测厚检查","管道硬度检查","金相分析",
			"压力试验","泄漏性检查","管道安全等级评估","管道全面检验结论"};// 页签对应的数据
	
	public PipeOverallCheckPager(Activity activity) {
		super(activity);
	}
	...
	public void initData() {
		 ...
		 * 添加页签对应的页面
		 */
		pipeTabDetailPagers.add(new MenuDetailUI(mActivity, PipeOverallCheckData.getTab1Data()));
		pipeTabDetailPagers.add(new MenuDetailUI(mActivity, PipeOverallCheckData.getTab2Data()));
		pipeTabDetailPagers.add(new MenuDetailUI(mActivity, PipeOverallCheckData.getTab3Data()));
		pipeTabDetailPagers.add(new MenuDetailUI(mActivity, PipeOverallCheckData.getTab4Data()));
		pipeTabDetailPagers.add(new MenuDetailUI(mActivity, PipeOverallCheckData.getTab5Data()));
		pipeTabDetailPagers.add(new MenuDetailUI(mActivity, PipeOverallCheckData.getTab6Data()));
		pipeTabDetailPagers.add(new MenuDetailUI(mActivity, PipeOverallCheckData.getTab7Data()));
		pipeTabDetailPagers.add(new MenuDetailUI(mActivity, PipeOverallCheckData.getTab8Data()));
		pipeTabDetailPagers.add(new MenuDetailUI(mActivity, PipeOverallCheckData.getTab9Data()));
		/**
		 * 
		 */
