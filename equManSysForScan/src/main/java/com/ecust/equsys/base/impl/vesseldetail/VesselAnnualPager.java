...
import com.ecust.equsys.base.impl.vesseldetail.data.VesselAnnualData;
...
public class VesselAnnualPager extends MenuDetailBasePager implements ViewPager.OnPageChangeListener{
	...
	String[] indicatorItem = {"压力容器年度检查结论记录","压力容器年度检查记录附页"};// 页签对应的数据

	public VesselAnnualPager(Activity activity) {
		super(activity);
	}
	...
	public void initData() {
		 ...
		 * 添加页签对应的页面
		 */
		pipeTabDetailPagers.add(new MenuDetailUI(mActivity, VesselAnnualData.getTab1Data()));
		pipeTabDetailPagers.add(new MenuDetailUI(mActivity, VesselAnnualData.getTab2Data()));
		/**
		 * 
		 */
