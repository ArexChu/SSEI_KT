...
public class PipeTagLoadPager extends MenuDetailBasePager{
	...
	class MissionSubmitClickListener implements OnClickListener{
		...
		@Override
		public void onClick(View v) {
			missionID = et_missionID.getText().toString();
			if (missionID == null) {
				Toast.makeText(mActivity, "业务号不能为空！", Toast.LENGTH_SHORT).show();
			}
			else {
				CacheUtils.putString(mActivity, PIPE_MISSION_ID, missionID);
				et_missionID.setText(missionID);
	...
	class SearchClickListener implements OnClickListener{
		...
		@Override
		public void onClick(View v) {
			SearchTagDialog searchTag = new SearchTagDialog(mActivity);
